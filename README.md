...
| `JSContext *kugou_init(ProcessEnv *env);`           | 初始化酷狗 API 上下文     |
...
int main(int argc, char **argv)
{
  ...
  // 你也可以调用 JSContext *ctx = kugou_init_simple(); 所有参数将为空
  // 如果需要多个上下文如多线程使用，
  // 可以手动调用 get_kugou_context() 为每个线程创建独立 context
  // JSContext *ctx2 = get_kugou_context(); 创建第二个上下文
